...
vm.runInContext(fs.readFileSync('trade-engine.js','utf8'),ctx);
vm.runInContext(fs.readFileSync('single-sheet-export.js','utf8'),ctx);
let html=fs.readFileSync('index.html','utf8'),js=[...html.matchAll(/<script>([\s\S]*?)<\/script>/g)].at(-1)[1];
